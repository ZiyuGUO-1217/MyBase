...
class MyRestfulRepository @Inject constructor(
    ...
    override suspend fun getAllItems(): Result<List<RestfulObjectData>> =
        callApi { remoteDataSource.getAllObjects() }
            .mapCatchingToDomainError { response ->
                response.map(RestfulDataMapper::toRestfulObjectData)
            }

    override suspend fun getItemsById(ids: List<String>): Result<List<RestfulObjectData>> =
        callApi { remoteDataSource.getObjectsByIds(ids) }
            .mapCatchingToDomainError { response ->
                response.map(RestfulDataMapper::toRestfulObjectData)
            }
    ...
    override suspend fun getItem(id: String): Result<RestfulObjectData> =
        callApi { remoteDataSource.getObject(id) }
            .mapCatchingToDomainError(RestfulDataMapper::toRestfulObjectData)
}
